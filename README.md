...
- **知识库（边写边查）**：资料栏新增「知识库」tab，可连接本地 Markdown 文件夹（如 **Obsidian 库**；腾讯 ima 等云端库可先把笔记导出为本地 Markdown 再连接），只读列出全部 `.md` 笔记并渲染正文；搜索笔记名、点开即看，写稿同时随手翻资料。仅本机读取，不联网。
- **素材卡片**：随手存梗、金句、设定碎片。
