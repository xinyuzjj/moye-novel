...
- **知识库（边写边查）**：右侧栏只保留「知识库」，可连接本地 Markdown 文件夹（如 **Obsidian 库**；腾讯 ima 等云端库可先把笔记导出为本地 Markdown 再连接）。搜索笔记名、点开后直接在当前侧栏内阅读，不弹抽屉；侧栏宽度可拖拽调整（240px~520px），顺手存到本地。仅本机读取，不联网。
- **素材卡片**：随手存梗、金句、设定碎片。
