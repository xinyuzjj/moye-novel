...
globalThis.Neutralino = {
  os: {
    getPath: async (name) => (name === 'documents' ? DOCS.replace(/\\/g, '/') : null),
    showSaveDialog: async () => ({ filePath: path.join(OUT, 'book.txt').replace(/\\/g, '/') }),
    showOpenDialog: async () => [path.join(OUT, 'in.json').replace(/\\/g, '/')],
  },
  filesystem: {
    getStats: async (p) => {
